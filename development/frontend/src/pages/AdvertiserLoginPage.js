    ...
    <div style={{ minHeight: '100vh', background: 'var(--white)' }}>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 80px)' }}>
        <div style={{ 
          ...
          <h1 style={{ fontSize: '3rem', fontWeight: '800', marginBottom: '20px' }}>Creative<br/><span style={{ color: 'var(--crimson)' }}>ADVERTISER HUB</span></h1>
          <p style={{ fontSize: '1.2rem', opacity: 0.8 }}>Secure entrance for marketing partners and ad managers.</p>
        </div>
          ...
          <div style={{ width: '100%', maxWidth: '400px' }}>
            ...
            <form onSubmit={handleLogin} style={{ display: 'grid', gap: '20px' }}>
              ...
              <button type="submit" disabled={loading} className="btn btn-crimson" style={{ padding: '16px', borderRadius: '12px', justifyContent: 'center' }}>
                {loading ? 'Authenticating...' : 'Access My Hub'}
              </button>
            </form>

            <div style={{ marginTop: '30px', textAlign: 'center', color: '#666', fontSize: '0.9rem' }}>
              New Advertising Partner? <Link to="/register?role=advertiser" style={{ color: 'var(--navy)', fontWeight: '800', textDecoration: 'none' }}>Register Hub</Link>
            </div>
          </div>
        </div>
      </div>
